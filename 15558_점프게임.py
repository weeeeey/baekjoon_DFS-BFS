...
0
'''
# 간단한 문제였는데 시간 거는 체크에서 0을 -1로 시작해버림
...
if __name__=='__main__':
    ...
    q = deque()
    q.append((0,0,0))
    visited = [[False]*n for i in range(2)]
    visited[0][0]=True 
    dx = [1,-1,k]
    result = 0
    while(q):
        line,cur,t = q.popleft()
        for i in range(3):
            tl = line
            if i==2:
                tl = 1 if tl==0 else 0
            ...
            if next>=n:
                result=1
                break
            if visited[tl][next]==False and gr[tl][next]=='1':

                visited[tl][next]=True 
                q.append((tl,next,t+1))
        if result==1:
            break
    print(result)
